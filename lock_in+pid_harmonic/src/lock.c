...
int lock_update_main(rp_app_params_t *params)
{
    ...
    params[111].value = (float)g_lock_reg->sg_amp1               ; // lock_sg_amp1
    params[112].value = (float)g_lock_reg->sg_amp2               ; // lock_sg_amp2
    params[113].value = (float)g_lock_reg->sg_amp3               ; // lock_sg_amp3
    params[114].value = (float) ((g_lock_reg->lpf_F1      )& 0x0f) ; // lock_lpf_F1_tau
    params[115].value = (float) ((g_lock_reg->lpf_F1 >> 4 )& 0x03) ; // lock_lpf_F1_order
    params[116].value = (float) ((g_lock_reg->lpf_F2      )& 0x0f) ; // lock_lpf_F2_tau
    params[117].value = (float) ((g_lock_reg->lpf_F2 >> 4 )& 0x03) ; // lock_lpf_F2_order
    params[118].value = (float) ((g_lock_reg->lpf_F3      )& 0x0f) ; // lock_lpf_F3_tau
    params[119].value = (float) ((g_lock_reg->lpf_F3 >> 4 )& 0x03) ; // lock_lpf_F3_order
    params[120].value = (float)g_lock_reg->error_sw              ; // lock_error_sw
    params[121].value = (float)g_lock_reg->error_offset          ; // lock_error_offset
    params[122].value = (float)g_lock_reg->error                 ; // lock_error
    params[123].value = ((float) ( g_lock_reg->error_mean >= 0 ? g_lock_reg->error_mean : g_lock_reg->error_mean-32 ))/262144  ; // lock_error_mean
    lock_error_var    = ((float) (g_lock_reg->error_std))/32 - pow(params[LOCK_ERROR_MEAN].value,2) ;
    params[124].value = lock_error_var<0 ? -1 : sqrt( lock_error_var )  ; // lock_error_std
    params[125].value = (float)g_lock_reg->mod_out1              ; // lock_mod_out1
    params[126].value = (float)g_lock_reg->mod_out2              ; // lock_mod_out2
    params[127].value = (float)g_lock_reg->gen_mod_phase         ; // lock_gen_mod_phase
    params[128].value = (float)g_lock_reg->gen_mod_hp            ; // lock_gen_mod_hp
    params[129].value = (float)g_lock_reg->ramp_A                ; // lock_ramp_A
    params[130].value = (float)g_lock_reg->ramp_B                ; // lock_ramp_B
    params[131].value = (float)g_lock_reg->ramp_step             ; // lock_ramp_step
    params[132].value = (float)g_lock_reg->ramp_low_lim          ; // lock_ramp_low_lim
    params[133].value = (float)g_lock_reg->ramp_hig_lim          ; // lock_ramp_hig_lim
    params[134].value = (float)g_lock_reg->ramp_reset            ; // lock_ramp_reset
    params[135].value = (float)g_lock_reg->ramp_enable           ; // lock_ramp_enable
    params[136].value = (float)g_lock_reg->ramp_direction        ; // lock_ramp_direction
    params[137].value = (float)g_lock_reg->ramp_B_factor         ; // lock_ramp_B_factor
    params[138].value = (float)g_lock_reg->sin_ref               ; // lock_sin_ref
    params[139].value = (float)g_lock_reg->cos_ref               ; // lock_cos_ref
    params[140].value = (float)g_lock_reg->cos_1f                ; // lock_cos_1f
    params[141].value = (float)g_lock_reg->cos_2f                ; // lock_cos_2f
    params[142].value = (float)g_lock_reg->cos_3f                ; // lock_cos_3f
    params[143].value = (float)g_lock_reg->in1                   ; // lock_in1
    params[144].value = (float)g_lock_reg->in2                   ; // lock_in2
    params[145].value = (float)g_lock_reg->out1                  ; // lock_out1
    params[146].value = (float)g_lock_reg->out2                  ; // lock_out2
    params[147].value = (float)g_lock_reg->oscA                  ; // lock_oscA
    params[148].value = (float)g_lock_reg->oscB                  ; // lock_oscB
    lock_freeze_regs();
    params[149].value = (float)g_lock_reg->X_28                  ; // lock_X
    params[150].value = (float)g_lock_reg->Y_28                  ; // lock_Y
    params[151].value = (float)g_lock_reg->F1_28                 ; // lock_F1
    params[152].value = (float)g_lock_reg->F2_28                 ; // lock_F2
    params[153].value = (float)g_lock_reg->F3_28                 ; // lock_F3
    params[154].value = (float)g_lock_reg->cnt_clk               ; // lock_cnt_clk
    params[155].value = (float)g_lock_reg->cnt_clk2              ; // lock_cnt_clk2
    lock_restore_regs();
    params[156].value = (float)g_lock_reg->read_ctrl             ; // lock_read_ctrl
    params[157].value = (float)g_lock_reg->pidA_sw               ; // lock_pidA_sw
    params[158].value = (float)g_lock_reg->pidA_PSR              ; // lock_pidA_PSR
    params[159].value = (float)g_lock_reg->pidA_ISR              ; // lock_pidA_ISR
    params[160].value = (float)g_lock_reg->pidA_DSR              ; // lock_pidA_DSR
    params[161].value = (float)g_lock_reg->pidA_SAT              ; // lock_pidA_SAT
    params[162].value = (float)g_lock_reg->pidA_sp               ; // lock_pidA_sp
    params[163].value = (float)g_lock_reg->pidA_kp               ; // lock_pidA_kp
    params[164].value = (float)g_lock_reg->pidA_ki               ; // lock_pidA_ki
    params[165].value = (float)g_lock_reg->pidA_kd               ; // lock_pidA_kd
    params[166].value = (float)g_lock_reg->pidA_in               ; // lock_pidA_in
    params[167].value = (float)g_lock_reg->pidA_out              ; // lock_pidA_out
    params[168].value = (float) ((g_lock_reg->pidA_ctrl                 )& 0x001) ; // lock_pidA_irst
    params[169].value = (float) ((g_lock_reg->pidA_ctrl            >>1  )& 0x001) ; // lock_pidA_freeze
    params[170].value = (float) ((g_lock_reg->pidA_ctrl            >>2  )& 0x001) ; // lock_pidA_ifreeze
    params[171].value = (float)g_lock_reg->ctrl_A                ; // lock_ctrl_A
    params[172].value = (float)g_lock_reg->pidB_sw               ; // lock_pidB_sw
    params[173].value = (float)g_lock_reg->pidB_PSR              ; // lock_pidB_PSR
    params[174].value = (float)g_lock_reg->pidB_ISR              ; // lock_pidB_ISR
    params[175].value = (float)g_lock_reg->pidB_DSR              ; // lock_pidB_DSR
    params[176].value = (float)g_lock_reg->pidB_SAT              ; // lock_pidB_SAT
    params[177].value = (float)g_lock_reg->pidB_sp               ; // lock_pidB_sp
    params[178].value = (float)g_lock_reg->pidB_kp               ; // lock_pidB_kp
    params[179].value = (float)g_lock_reg->pidB_ki               ; // lock_pidB_ki
    params[180].value = (float)g_lock_reg->pidB_kd               ; // lock_pidB_kd
    params[181].value = (float)g_lock_reg->pidB_in               ; // lock_pidB_in
    params[182].value = (float)g_lock_reg->pidB_out              ; // lock_pidB_out
    params[183].value = (float) ((g_lock_reg->pidB_ctrl                 )& 0x001) ; // lock_pidB_irst
    params[184].value = (float) ((g_lock_reg->pidB_ctrl            >>1  )& 0x001) ; // lock_pidB_freeze
    params[185].value = (float) ((g_lock_reg->pidB_ctrl            >>2  )& 0x001) ; // lock_pidB_ifreeze
    params[186].value = (float)g_lock_reg->ctrl_B                ; // lock_ctrl_B
    params[187].value = (float)g_lock_reg->aux_A                 ; // lock_aux_A
    params[188].value = (float)g_lock_reg->aux_B                 ; // lock_aux_B
    params[189].value = (float) (( g_lock_reg->lock_feedback >> 0 ) & 0x01 ) ; // lock_ctrl_aux_lock_now
    params[190].value = (float) (( g_lock_reg->lock_feedback >> 1 ) & 0x01 ) ; // lock_ctrl_aux_launch_lock_trig
    params[191].value = (float) (( g_lock_reg->lock_feedback >> 2 ) & 0x01 ) ; // lock_ctrl_aux_pidB_enable_ctrl
    params[192].value = (float) (( g_lock_reg->lock_feedback >> 3 ) & 0x01 ) ; // lock_ctrl_aux_pidA_enable_ctrl
    params[193].value = (float) (( g_lock_reg->lock_feedback >> 4 ) & 0x01 ) ; // lock_ctrl_aux_ramp_enable_ctrl
    params[194].value = (float) (( g_lock_reg->lock_feedback >> 5 ) & 0x01 ) ; // lock_ctrl_aux_set_pidB_enable
    params[195].value = (float) (( g_lock_reg->lock_feedback >> 6 ) & 0x01 ) ; // lock_ctrl_aux_set_pidA_enable
    params[196].value = (float) (( g_lock_reg->lock_feedback >> 7 ) & 0x01 ) ; // lock_ctrl_aux_set_ramp_enable
    params[197].value = (float) (( g_lock_reg->lock_feedback >> 8 ) & 0x03 ) ; // lock_ctrl_aux_trig_type
    params[198].value = (float) (( g_lock_reg->lock_feedback >>10 ) & 0x01 ) ; // lock_ctrl_aux_lock_trig_rise
